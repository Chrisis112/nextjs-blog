        ...
        >
          ...
          >
            <div className="items-center flex flex-col">
              ...
              <h2 className="text-lg font-bold text-center mb-2">{menuItemTitle}</h2>
...
export default function MenuItem(menuItem) {
  ...
  const menuItemTitle = getLocalizedText(name);
  const menuItemDescription = getLocalizedText(description);
  const imageSrc = getLocalizedText(image) || "/default-image.png";

  // Убираем выбор локаций и параметров, только добавление в корзину по клику
  const handleAddToCart = () => {
    addToCart(
      { ...menuItem },
      null,   // selectedSize
      [],     // selectedExtras
      null,   // selectedTemperature
      ""      // selectedLocation (если не нужен)
    );
  };

  // Важно: передаём название категории (например, `categoryName`) в MenuItemTile
  // В MenuPage это поле приходит в `categories[i].name`
  return (
    <MenuItemTile
      ...
      image={imageSrc}
      title={menuItemTitle}
      categoryName={menuItem.categoryName} // ← добавили проп

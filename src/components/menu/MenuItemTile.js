...
export default function MenuItemTile({ onAddToCart, ...item }) {
  ...
  const categoryName = item.categoryName ? getLocalizedText(item.categoryName) : "";
  const imageSrc = getLocalizedText(item.image) || "/default-image.png";
  ...
  return (
    <div
      ...
      style={{
        width: "100%",
        maxWidth: "400px",
        minHeight: "450px",     // можно уменьшить или увеличить
        boxSizing: "border-box",
      }}
    >
      ...
      >
        <Image
          src={imageSrc}
          alt={categoryName || "Menu Image"}
          fill
          style={{
            objectFit: "contain",  // 🔥 Важно: picture не обрезается, помещается целиком
          }}
          sizes="(max-width: 768px) 90vw, 320px"
        />
      </div>

      {/* Название категории (можно убрать, если не нужно) */}
    </div>
  );
}
    ...
    if (typeof admin.messaging().sendEachForMulticast === 'function') {
      ...
      if (response.failureCount > 0) {
        ...
        response.responses.forEach((resp, idx) => {
          if (!resp.success) {
            console.error(`Failed to send to token ${validTokens[idx].slice(0, 20)}...:`, resp.error?.code || resp.error);
          }
        });
      }
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (isPaid) {
        ...
        try {
          ...
          console.log(`Found ${sellers.length} sellers with ${allFcmTokens.length} total FCM tokens`);

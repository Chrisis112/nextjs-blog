...
export default function MenuPage() {
  ...
  useEffect(() => {
    // добавь обработку ошибок, чтобы видеть, если /api/ падает
    fetch('/api/categories')
      .then((res) => res.json())
      .then((data) => {
  ...
  // используем уже `title` (как объект {ru,en,et}) вместо `name`
  const getCategoryName = (name) => {
    if (!name) return '';
        ...
        <section className="mt-8 space-y-12">
          {categories.map((c) => {
            const itemsFilteredByCategoryAndLocation = menuItems
              .filter(
                (item) =>
                  String(item.category) === String(c._id) &&
            ...
            return (
              <div key={c._id}>
                <div className="text-center">
                  <SectionHeaders
                    mainHeader={getCategoryName(c.name)}
                  />
                </div>
                <div
                  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center"
